...
#>
    ...
    $playlistContent += "# Generated by MuFo for manual track mapping"
    $playlistContent += "# Play this playlist while editing the mapping file"
    $playlistContent += ""
    
    foreach ($track in $sortedTracks) {
        # Standard M3U format with duration and title info
    ...
    Write-Host ""
    # Determine if we should open files automatically
    $shouldOpen = if ($OpenFiles) { $true } elseif ($NoOpen) { $false } else {
        # Auto-open by default unless in non-interactive mode
        $Host.UI.RawUI -and -not $env:CI -and -not $env:AUTOMATION
    }
    
    if ($shouldOpen) {
        Write-Host "🎵 Opening playlist in default media player..." -ForegroundColor Cyan
        try {
            Start-Process -FilePath $playlistPath -ErrorAction Stop
            Write-Host "✅ Playlist opened: $playlistPath" -ForegroundColor Green
        } catch {
            Write-Warning "Could not open playlist automatically: $($_.Exception.Message)"
            Write-Host "💡 Manually open: $playlistPath" -ForegroundColor Yellow
        }
        
        Write-Host "📝 Opening mapping file in text editor..." -ForegroundColor Cyan
        try {
            # Try environment editor first, then fallback to system default
            if ($env:EDITOR) {
                Start-Process -FilePath $env:EDITOR -ArgumentList $mappingPath -ErrorAction Stop
                Write-Host "✅ Mapping file opened in \$env:EDITOR: $mappingPath" -ForegroundColor Green
            } else {
                # Use Invoke-Item to open with system default text editor
                Invoke-Item -Path $mappingPath
                Write-Host "✅ Mapping file opened with default editor: $mappingPath" -ForegroundColor Green
            }
        } catch {
            Write-Warning "Could not open mapping file automatically: $($_.Exception.Message)"
            Write-Host "💡 Manually edit: $mappingPath" -ForegroundColor Yellow
        }
        
        Write-Host ""
        Write-Host "Next steps:" -ForegroundColor Yellow
        Write-Host "1. 🎵 Listen to the playlist (should be playing now)" -ForegroundColor White
        Write-Host "2. ✏️  Edit the mapping file (should be open now) - reorder to match what you hear" -ForegroundColor White

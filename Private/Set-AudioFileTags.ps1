...
.SYNOPSIS
    ...
    if ($existingTags.Count -eq 0) {
        Write-Warning "No au            # Fetch album trac                if ($spotifyTrack -and $spotifyTrack.artists -and $spotifyTrack.artists.Count -gt 0) {
                    ...
                    $rawTrackArtistNames = $spotifyTrack.artists | ForEach-Object { $_.name }
                    Write-Host "DEBUG: Raw Artists array: $($rawTrackArtistNames -join ' | ')" -ForegroundColor Magenta
                    $trackArtistNames = $rawTrackArtistNames | Sort-Object -Unique
                    Write-Host "DEBUG: After dedup: $($trackArtistNames -join ' | ')" -ForegroundColor Cyan
                    $suggestedArtist = $trackArtistNames -join ', '
                    Write-Verbose "Track $($track.Track): Found track artists: $suggestedArtist"ot already available in SpotifyAlbum
            if ($SpotifyAlbum -and -not $SpotifyAlbum.tracks) {
                try {
                    ...
                    Write-Host "DEBUG: Fetched $($albumTracks.Count) tracks" -ForegroundColor Green
                    Write-Verbose "Fetched $($albumTracks.Count) tracks for album '$($SpotifyAlbum.name)'"
                } catch {
                    Write-Host "DEBUG: Failed to fetch tracks: $($_.Exception.Message)" -ForegroundColor Red
                    Write-Verbose "Could not fetch album tracks: $($_.Exception.Message)"
                }
            } else {
                Write-Host "DEBUG: Tracks already available: $($SpotifyAlbum.tracks.items.Count)" -ForegroundColor Blue
            }und or could not read existing tags"
        return @()
    }
...
#>
    ...
    # Always populate from Spotify data when available (for format-separated folders where track metadata may be incomplete)
    if ($SpotifyAlbum) {
        if ($SpotifyAlbum.name -and (-not $albumAnalysis.AlbumName -or $albumAnalysis.AlbumName -eq '')) {
            $albumAnalysis.AlbumName = $SpotifyAlbum.name
        }
        if ($SpotifyAlbum.artists -and $SpotifyAlbum.artists.Count -gt 0 -and (-not $albumAnalysis.ArtistName -or $albumAnalysis.ArtistName -eq '')) {
            $albumAnalysis.ArtistName = ($SpotifyAlbum.artists | ForEach-Object { $_.name }) -join ', '
        }
    }
        ...
        foreach ($track in $albumAnalysisTracks) {
            ...
            if ($SpotifyAlbum -and $SpotifyAlbum.tracks -and $SpotifyAlbum.tracks.items -and $track.Track -gt 0) {
                ...
                if ($spotifyTrack -and $spotifyTrack.artists -and $spotifyTrack.artists.Count -gt 0) {
                    $rawTrackArtistNames = $spotifyTrack.artists | ForEach-Object { $_.name }
                    Write-Verbose ("Track {0}: Raw Spotify track artists -> {1}" -f $track.Track, ($rawTrackArtistNames -join ' | '))
                    $trackArtistNames = $rawTrackArtistNames | Sort-Object -Unique
                    Write-Verbose ("Track {0}: Deduped Spotify track artists -> {1}" -f $track.Track, ($trackArtistNames -join ' | '))
                    $suggestedArtist = $trackArtistNames -join ', '
                }
            }
...
"@
    ...
    foreach ($track in $existingTags) {
        try {
            ...
            # Retry logic for file access and other temporary issues
            $fileObj = $null
            $maxRetries = 3
            ...
            for ($attempt = 1; $attempt -le $maxRetries; $attempt++) {
                try {
                    ...
                    $isFileAccessError = $_.Exception.Message -match "being used by another process|access denied|sharing violation|cannot access the file"
                    
                    if ($attempt -eq $maxRetries) {
                        # On final attempt, show detailed error for debugging
                        if ($isFileAccessError) {
                            Write-Warning "File '$($track.FileName)' appears to be locked or inaccessible after $maxRetries attempts"
                        } else {
                            Write-Warning "Failed to open '$($track.FileName)' after $maxRetries attempts: $($_.Exception.Message)"
                        }
                        throw  # Re-throw to be caught by outer catch block
                    }
                    
                    # Check if it's a file access error (likely file locked)
                    if ($isFileAccessError) {
                        Write-Verbose "File '$($track.FileName)' is locked (attempt $attempt/$maxRetries), retrying in ${retryDelay}s..."
                        Start-Sleep -Seconds $retryDelay
                        $retryDelay *= 2  # Exponential backoff
                    } else {
                        # For other errors, retry once but with shorter delay
                        if ($attempt -eq 1) {
                            Write-Verbose "Retrying '$($track.FileName)' due to error: $($_.Exception.Message)"
                            Start-Sleep -Milliseconds 500
                        } else {
                            throw  # Don't retry other errors more than once
                        }
                    }
                }
            }
            ...
            if ('TrackNumbers' -in $tagsToFix -and ($tag.Track -eq 0 -or -not $tag.Track)) {
                ...
                # First, try to get track number from Spotify data by matching titles
                if ($SpotifyAlbum -and $SpotifyAlbum.tracks -and $SpotifyAlbum.tracks.items) {
                    # Find the best matching Spotify track by title similarity
                    $bestMatch = $null
                    $bestSimilarity = 0
                    
                    foreach ($spotifyTrack in $SpotifyAlbum.tracks.items) {
                        $similarity = Get-StringSimilarity -String1 $track.Title -String2 $spotifyTrack.name
                        if ($similarity -gt $bestSimilarity -and $similarity -gt 0.6) {  # 60% similarity threshold
                    ...
                    if ($bestMatch) {
                        $suggestedTrackNumber = $bestMatch.track_number
                        Write-Verbose "Matched '$($track.Title)' to Spotify track '$($bestMatch.name)' (similarity: $([math]::Round($bestSimilarity, 2))) -> Track $suggestedTrackNumber"
                    }
                }
                
                # If no Spotify match, try to extract from filename
                if (-not $suggestedTrackNumber) {
                    $filename = [System.IO.Path]::GetFileNameWithoutExtension($track.FileName)
                    if ($filename -match '^(\d+)') {
                        $extractedNumber = [int]$matches[1]
                        # Only use extracted number if it seems reasonable (not all tracks having same number)
                        $otherTracksSameNumber = $existingTags | Where-Object { 
                            $otherFileName = [System.IO.Path]::GetFileNameWithoutExtension($_.FileName)
                            $otherFileName -match '^(\d+)' -and [int]$matches[1] -eq $extractedNumber
                        }
                        
                        # If multiple tracks have the same number, it's likely wrong - don't use it
                        if ($otherTracksSameNumber.Count -le 1) {
                            $suggestedTrackNumber = $extractedNumber
                        } else {
                            Write-Verbose "Detected multiple tracks with same number ($extractedNumber) from filename - will use file order instead"
                        }
                    }
                }
                
                # Final fallback: use track duration matching or file order
                if (-not $suggestedTrackNumber) {
                    # Try duration matching with Spotify data first
                    $trackDurationSeconds = $null
                    if ($null -ne $track.DurationSeconds) {
                        if ($track.DurationSeconds -is [double] -or $track.DurationSeconds -is [single] -or $track.DurationSeconds -is [decimal] -or $track.DurationSeconds -is [int] -or $track.DurationSeconds -is [long]) {
                            $trackDurationSeconds = [double]$track.DurationSeconds
                        }
                        else {
                            $durationString = $track.DurationSeconds.ToString()
                            if (-not [string]::IsNullOrWhiteSpace($durationString)) {
                                $parsedDuration = 0.0
                                if ([double]::TryParse($durationString, [System.Globalization.NumberStyles]::Float, [System.Globalization.CultureInfo]::InvariantCulture, [ref]$parsedDuration)) {
                                    $trackDurationSeconds = $parsedDuration
                                }
                            }
                        }
                    }

                    if ($SpotifyAlbum -and $SpotifyAlbum.tracks -and $SpotifyAlbum.tracks.items -and $trackDurationSeconds -ne $null -and $trackDurationSeconds -gt 0) {
                        ...
                        # Categorize track by duration for appropriate tolerance
                        $trackCategory = if ($trackDurationSeconds -lt 120) { "Short" }
                                        elseif ($trackDurationSeconds -lt 420) { "Normal" }
                                        elseif ($trackDurationSeconds -lt 600) { "Long" }
                                        else { "Epic" }
                        
                        # Use data-driven tolerance for this track category
                        $toleranceSeconds = $dataDrivenTolerances[$trackCategory].Normal
                        
                        # Find Spotify tracks within the data-driven tolerance
                        $matchingDurationTracks = $SpotifyAlbum.tracks.items | Where-Object {
                            [math]::Abs($_.duration_ms / 1000 - $trackDurationSeconds) -le $toleranceSeconds
                        } | Sort-Object { [math]::Abs($_.duration_ms / 1000 - $trackDurationSeconds) }
                        
                        if ($matchingDurationTracks -and $matchingDurationTracks.Count -gt 0) {
                            # Use the closest duration match
                            $bestDurationMatch = $matchingDurationTracks[0]
                            $durationDiff = [math]::Abs($bestDurationMatch.duration_ms / 1000 - $trackDurationSeconds)
                            
                            # Only accept matches within reasonable bounds (should be within tolerance, but double-check)
                            if ($durationDiff -le ($toleranceSeconds * 1.5)) {
                                $suggestedTrackNumber = $bestDurationMatch.track_number
                                Write-Verbose "Matched '$($track.FileName)' (duration: $([math]::Round($trackDurationSeconds, 0))s, category: $trackCategory) to Spotify track '$($bestDurationMatch.name)' (duration: $([math]::Round($bestDurationMatch.duration_ms / 1000, 0))s, diff: $([math]::Round($durationDiff, 0))s, tolerance: ${toleranceSeconds}s) -> Track $suggestedTrackNumber"
                            }
                        }
                    }
                    
                    # If no duration match, fall back to file order
                    if (-not $suggestedTrackNumber) {
                        ...
                        if ($index -ge 0) {
                            ...
                            Write-Verbose "Assigned track number $suggestedTrackNumber based on file order for '$($track.FileName)'"
                        }
                    }
                }
            ...
            if ('TrackArtists' -in $tagsToFix -and $suggestedArtist -and $suggestedArtist -ne $track.Artist) {
                Write-Verbose "TrackArtist change needed for '$($track.FileName)': '$suggestedArtist' != '$($track.Artist)'"
                if (-not $WhatIfPreference) {
            ...
            } elseif ('TrackArtists' -in $tagsToFix -and $suggestedArtist) {
                Write-Verbose "TrackArtist already correct for '$($track.FileName)': '$suggestedArtist' == '$($track.Artist)'"
            } else {
                Write-Verbose "TrackArtist not processed for '$($track.FileName)': tagsToFix=$('TrackArtists' -in $tagsToFix), suggestedArtist='$suggestedArtist'"
            }
            ...
            if ('AlbumArtists' -in $tagsToFix -and $suggestedAlbumArtist -and $suggestedAlbumArtist -ne $track.AlbumArtist) {
                Write-Verbose "AlbumArtist change needed for '$($track.FileName)': '$suggestedAlbumArtist' != '$($track.AlbumArtist)'"
                if (-not $WhatIfPreference) {
            ...
            } elseif ('AlbumArtists' -in $tagsToFix -and $suggestedAlbumArtist) {
                Write-Verbose "AlbumArtist already correct for '$($track.FileName)': '$suggestedAlbumArtist' == '$($track.AlbumArtist)'"
            }
            ...
            if ($changesMadeToFile) {
                if (-not $WhatIfPreference) {
                    try {
                        ...
                        Write-Warning "Failed to save changes to '$($track.FileName)': $($_.Exception.Message)"
                        $changesMadeToFile = $false
                        $result = [PSCustomObject]@{
                            Path = $track.Path
                            FileName = $track.FileName
                            ChangesApplied = @("ERROR: Failed to save - $($_.Exception.Message)")
                            Success = $false
                ...
                # No changes needed - this is actually successful
                Write-Verbose "No changes needed for '$($track.FileName)'"
            }            # Create result object for this track
            if ($changesMadeToFile -and -not $WhatIfPreference) {
                $result = [PSCustomObject]@{
                    Path = $track.Path
                    FileName = $track.FileName
            ...
            } elseif ($changesMadeToFile -and $WhatIfPreference) {
                # WhatIf mode - still count as success for reporting
                $result = [PSCustomObject]@{
                    Path = $track.Path
                    FileName = $track.FileName
                    ChangesApplied = $changes
                ...
                # Changes identified but not applied (warnings, manual review needed)
                $result = [PSCustomObject]@{
                    Path = $track.Path
                    FileName = $track.FileName
                    ChangesApplied = $changes
                ...
                $result = [PSCustomObject]@{
                    Path = $track.Path
                    FileName = $track.FileName
                    ChangesApplied = @("No changes needed")
                    Success = $true
                    Error = $null
                }
            }
            ...
            Write-Verbose "Error details: $($_.Exception.GetType().Name) - $($_.Exception.StackTrace)"
    ...
    if ($failedFiles) {
        ...
        Write-Host "  💡 Failed files may be locked by other applications (media players, etc.)" -ForegroundColor Yellow
        Write-Host "     Try closing media applications and re-running the command" -ForegroundColor Yellow
    }
    ...
    if ($WhatIfPreference) {
        ...
        Write-Host "  Files updated: $changesMade" -ForegroundColor Green
        Write-Host "  Files unchanged: $($existingTags.Count - $changesMade - $failedFiles.Count)" -ForegroundColor Gray
    }
    ...
    # Return both results and album analysis for display purposes

        ...
        try {
            ...
            if ('TrackNumbers' -in $tagsToFix -and ($tag.Track -eq 0 -or -not $tag.Track)) {
                ...
                if (-not $suggestedTrackNumber) {
                    ...
                    if ($SpotifyAlbum -and $SpotifyAlbum.tracks -and $SpotifyAlbum.tracks.items -and $track.Duration -gt 0) {
                        # Find tracks with similar duration (±30 seconds tolerance)
                        $durationTolerance = 30  # seconds
                        $matchingDurationTracks = $SpotifyAlbum.tracks.items | Where-Object {
                            [math]::Abs($_.duration_ms / 1000 - $track.Duration) -le $durationTolerance
                        } | Sort-Object { [math]::Abs($_.duration_ms / 1000 - $track.Duration) }
                        ...
                        if ($matchingDurationTracks -and $matchingDurationTracks.Count -gt 0) {
                            ...
                            $durationDiff = [math]::Abs($bestDurationMatch.duration_ms / 1000 - $track.Duration)
                            
                            # Only use if the difference is reasonable (< 45 seconds)
                            if ($durationDiff -le 45) {
                                $suggestedTrackNumber = $bestDurationMatch.track_number
                                Write-Verbose "Matched '$($track.FileName)' (duration: $([math]::Round($track.Duration, 0))s) to Spotify track '$($bestDurationMatch.name)' (duration: $([math]::Round($bestDurationMatch.duration_ms / 1000, 0))s, diff: $([math]::Round($durationDiff, 0))s) -> Track $suggestedTrackNumber"
                            }
                        }
                    }

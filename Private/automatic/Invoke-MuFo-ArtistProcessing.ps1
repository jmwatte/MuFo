function Invoke-MuFoArtistProcessing {
    ...
    if ($DoIt -eq 'Manual') {
        $res = Get-MuFoManualSolution -Path $artistPath -IncludeTracks
        ...
        if ($topMatches) {
            ...
            if ($selectedArtist) {
                ...
                try {
                    # Handle Manual mode with interactive album/track selection
                    if ($DoIt -eq 'Manual') {
                        ...
                        $albumProcessingResult = Invoke-MuFoAlbumProcessing -AlbumComparisons $albumComparisons -SelectedArtist $selectedArtist -CurrentPath $artistPath -IncludeTracks:$IncludeTracks -BoxMode:$BoxMode
                    
                        if ($albumProcessingResult.Action -eq 'back') {
                            # Go back to artist selection
                            continue
                        }
                        elseif ($albumProcessingResult.Action -eq 'quit') {
                            # Quit the entire operation
                            return
                        }
                    
                        # For Manual mode, we only process the selected album and tracks
                        $selectedAlbum = $albumProcessingResult.SelectedAlbum
                        $selectedTracks = $albumProcessingResult.SelectedTracks
                    
                        if (-not $selectedAlbum) {
                            Write-Host "No album selected. Skipping this artist." -ForegroundColor Yellow
                            continue
                        }
                    
                        # Create a single-item array with just the selected album
                        $albumComparisons = @($selectedAlbum)
                    
                        # If tracks were selected, filter the album comparison to only include selected tracks
                        if ($selectedTracks -and $selectedTracks.Count -gt 0) {
                            $selectedAlbum | Add-Member -NotePropertyName SelectedTracks -NotePropertyValue $selectedTracks -Force
                        }
                    }
                    else {
                        # Use standard album processing logic for Automatic/Smart modes
                        $albumComparisons = Get-AlbumComparisons -CurrentPath $artistPath -SelectedArtist $selectedArtist -EffectiveExclusions $effectiveExclusions -ForcedAlbum $forcedAlbumWrapper
                    }
                    ...
                    if ($IncludeTracks) {
                        ...
                        foreach ($c in $albumComparisons) {
                            try {
                                ...
                                if ($DoIt -eq 'Manual' -and $c.SelectedTracks -and $c.SelectedTracks.Count -gt 0) {
                                    $selectedTrackPaths = $c.SelectedTracks | Select-Object -ExpandProperty Path
                                    $tracks = $tracks | Where-Object { $_.Path -in $selectedTrackPaths }
                                    Write-Verbose "Filtered to $($tracks.Count) selected tracks for Manual mode"
                                }
